  ...
  return (
    <div className={`p-8 rounded-[32px] border transition-all duration-500 relative overflow-hidden group
      ...
      <div className="flex items-end gap-2 relative z-10">
        <div className="text-4xl font-black tracking-tighter transition-colors">{value}</div>
        <div className={`text-[10px] font-bold mb-1.5 uppercase tracking-widest transition-colors ${black ? 'text-gray-500' : 'text-gray-300 group-hover:text-black'}`}>
          {unit}
        </div>
      </div>

      {/* Deco background */}
      <Icon size={120} className={`absolute -bottom-10 -right-10 opacity-[0.03] group-hover:opacity-[0.08] transition-all duration-500 group-hover:scale-110 pointer-events-none ${black ? 'text-white' : 'text-black'}`} />
    </div>
  )
}
