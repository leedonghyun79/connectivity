...
import { getDailyStats } from '@/lib/actions';
import {
  AreaChart,
  Area,
  XAxis,
  ...
  useEffect(() => {
    getDailyStats().then(stats => {
      ...
      const chartData = stats.map(s => ({
        date: new Date(s.date).toLocaleDateString('ko-KR', { month: '2-digit', day: '2-digit' }),
        pv: s.pageViews,
        visitor: s.visitors,
      })).reverse(); // 시간순 정렬
      setData(chartData);
  ...
  return (
    <div className="bg-white p-10 rounded-[40px] border border-gray-100 h-[480px] flex flex-col group">
      ...
      <div className="flex-1 min-h-0 w-full">
        {isLoading ? (
          <div className="h-full flex items-end gap-3 pb-6">
          ...
          <div className="h-full flex flex-col items-center justify-center gap-4 opacity-20">
            <TrendingUp size={48} />
            <p className="text-sm font-black uppercase tracking-[0.3em]">데이터 분석 중</p>
          </div>
        ) : (
