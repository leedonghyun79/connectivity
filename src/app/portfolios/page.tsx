...
import { Plus, Pencil, Trash2, Send, Undo2, Loader2, Briefcase } from 'lucide-react';
import { toast } from 'sonner';
import { getPortfolios, deletePortfolio, publishPortfolio, unpublishPortfolio } from '@/lib/portfolio-actions';
...
export default function PortfoliosPage() {
  ...
  const load = async () => {
    setLoading(true);
    setRows((await getPortfolios()) as Portfolio[]);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const doPublish = async (id: string) => {
    setBusyId(id);
    const res = await publishPortfolio(id);
    setBusyId(null);
  ...
  const doDelete = async () => {
    if (!deleteId) return;
    setBusyId(deleteId);
    const res = await deletePortfolio(deleteId);
    setBusyId(null);
    setDeleteId(null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-black">작업물 관리</h1>
          <p className="mt-1 text-sm text-gray-400">픽셀커넥트 공개 사이트 포트폴리오에 발행하는 작업물</p>
        </div>
        <Link
          href="/portfolios/new"
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="flex h-48 flex-col items-center justify-center gap-2 text-gray-400">
            ...
            <p className="text-sm">아직 등록한 작업물이 없습니다.</p>
          </div>
        ) : (
                ...
                return (
                  <tr key={r.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60">
                    <td className="px-6 py-4">
                      <Link href={`/portfolios/${r.id}/edit`} className="font-semibold text-black hover:underline">
                        {r.title || '(제목 없음)'}
                      </Link>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => router.push(`/portfolios/${r.id}/edit`)}
      ...
      <ConfirmModal
        ...
        onConfirm={doDelete}
        title="작업물 삭제"
        message="이 작업물을 삭제하시겠습니까? 발행된 경우 공개 사이트에서도 제거됩니다."
        confirmText="삭제"
        type="danger"
      />
    </div>
